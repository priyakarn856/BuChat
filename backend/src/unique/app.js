  ...
  try {
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.name && path.includes("/polls")) {
      ...
      if (!userId || !question || !options || options.length < 2) {
        return createResponse(400, { message: "userId, question, and at least 2 options required" });
      }
      ...
      return createResponse(201, { pollId, expiresAt, createdAt: now });
    }
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.pollId && path.includes("/vote")) {
      ...
      if (!userId || !optionIds || optionIds.length === 0) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "userId and optionIds required" }) };
      }
      ...
      if (!pollResult.Items || pollResult.Items.length === 0) {
        return { statusCode: 404, headers: getCorsHeaders(), body: JSON.stringify({ message: "poll not found" }) };
      }
      ...
      if (new Date(poll.expiresAt) < new Date()) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "poll has expired" }) };
      }

      // Check if multiple votes allowed
      if (!poll.allowMultiple && optionIds.length > 1) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "multiple votes not allowed" }) };
      }
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify({ message: "vote recorded", totalVotes })
      };
    }
    ...
    if (method === "GET" && event.pathParameters && event.pathParameters.pollId && path.includes("/polls/") && !path.includes("/vote")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify({
          ...poll,
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.name && path.includes("/capsules")) {
      ...
      if (!userId || !title || !message || !openDate) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "userId, title, message, and openDate required" }) };
      }
      ...
      if (new Date(openDate) <= new Date()) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "openDate must be in the future" }) };
      }
      ...
      return { statusCode: 201, headers: getCorsHeaders(), body: JSON.stringify({ 
          capsuleId, 
          openDate,
    ...
    if (method === "GET" && event.pathParameters && event.pathParameters.capsuleId && path.includes("/capsules/") && !path.includes("/upcoming")) {
      ...
      if (!result.Items || result.Items.length === 0) {
        return { statusCode: 404, headers: getCorsHeaders(), body: JSON.stringify({ message: "capsule not found" }) };
      }
      ...
      if (now < openDate && capsule.status === "sealed") {
        ...
        return { statusCode: 403, headers: getCorsHeaders(), body: JSON.stringify({ 
    ...
    if (method === "GET" && path.includes("/capsules/upcoming")) {
      ...
      const params = {
        TableName: TABLE,
        FilterExpression: "#type = :type AND #status = :status" + (group ? " AND group = :comm" : ""),
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify({ capsules: upcoming })
      };
    }
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.name && path.includes("/events") && !path.includes("/upcoming")) {
      ...
      if (!userId || !title || !eventDate) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "userId, title, and eventDate required" }) };
      }
      ...
      return { statusCode: 201, headers: getCorsHeaders(), body: JSON.stringify({ eventId, eventDate })
      };
    }
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.eventId && path.includes("/rsvp")) {
      ...
      if (!userId || !status) {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "userId and status required" }) };
      }
      ...
      if (!eventResult.Items || eventResult.Items.length === 0) {
        return { statusCode: 404, headers: getCorsHeaders(), body: JSON.stringify({ message: "event not found" }) };
      }
      ...
      if (eventItem.maxAttendees && eventItem.attendeeCount >= eventItem.maxAttendees && status === "going") {
        return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "event is full" }) };
      }
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify({ message: "RSVP updated", attendeeCount: attendees.length })
      };
    }
    ...
    if (method === "GET" && path.includes("/events/upcoming")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify({ events: upcoming })
      };
    }
    ...
    if (method === "GET" && event.pathParameters && event.pathParameters.eventId && path.includes("/events/") && !path.includes("/rsvp") && !path.includes("/upcoming")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(), body: JSON.stringify(eventItem)
      };
    }

    return { statusCode: 400, headers: getCorsHeaders(), body: JSON.stringify({ message: "bad request" }) };
  } catch (err) {
    console.error("Unique features error", err);
    return { statusCode: 500, headers: getCorsHeaders(), body: JSON.stringify({ message: "internal error", error: err.message }) };
  }
};
