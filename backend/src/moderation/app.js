...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.postId && path.includes("/report")) {
      ...
      if (!userId || !reason) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "userId and reason required" }) };
      }
      ...
      return { statusCode: 201, headers: getCorsHeaders(event), body: JSON.stringify({ reportId, message: "report submitted" })
      };
    }
    ...
    if (method === "POST" && path.includes("/hide")) {
      ...
      if (!moderatorId) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "moderatorId required" }) };
      }
      ...
      if (!result.Items || result.Items.length === 0) {
        return { statusCode: 404, headers: getCorsHeaders(event), body: JSON.stringify({ message: `${targetType} not found` }) };
      }
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ message: `${targetType} hidden` })
      };
    }
    ...
    if (method === "DELETE" && path.includes("/remove")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ message: `${targetType} removed` })
      };
    }

    // BAN USER FROM group - POST /communities/{name}/ban
    if (method === "POST" && event.pathParameters && event.pathParameters.name && path.includes("/ban")) {
      ...
      if (!userId || !moderatorId) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "userId and moderatorId required" }) };
      }
    ...
    if (method === "DELETE" && path.includes("/ban") && event.pathParameters.name && event.pathParameters.userId) {
      ...
      await ddb.send(new UpdateCommand({
        TableName: TABLE,
        Key: { PK: `GROUP#${group}`, SK: `BAN#${userId}` },
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ message: "user unbanned" })
      };
    }
    ...
    if (method === "GET" && path.includes("/ban") && event.pathParameters.name && event.pathParameters.userId) {
      ...
      if (!result.Item || result.Item.status !== "active") {
        return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ banned: false })
        };
      }
    ...
    if (method === "POST" && path.includes("/moderators") && event.pathParameters.name) {
      ...
      if (!userId || !assignedBy) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "userId and assignedBy required" }) };
      }
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ message: "moderator assigned" })
      };
    }
    ...
    if (method === "GET" && path.includes("/reports/pending")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ reports, count: reports.length })
      };
    }
    ...
    if (method === "PUT" && event.pathParameters && event.pathParameters.reportId) {
      ...
      if (!moderatorId || !status) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "moderatorId and status required" }) };
      }
      ...
      if (!result.Items || result.Items.length === 0) {
        return { statusCode: 404, headers: getCorsHeaders(event), body: JSON.stringify({ message: "report not found" }) };
      }
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ message: "report updated" })
      };
    }
    ...
    if (method === "GET" && path.includes("/moderation/queue") && event.pathParameters.name) {
      ...
      const params = {
        ...
        ExpressionAttributeValues: { ":pk": `GROUP#${group}` },
    ...
    if (method === "POST" && path.includes("/automod") && event.pathParameters.name) {
      ...
      if (!moderatorId || !rules) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "moderatorId and rules required" }) };
      }
