...
const BADGES = {
  ...
  group_FOUNDER: { id: "group_founder", name: "Founder", description: "Created a group", icon: "🏗️", rarity: "rare" },
  ...
  try {
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.username && path.includes("/badges")) {
      ...
      if (!badgeId || !BADGES[badgeId.toUpperCase()]) {
        return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ message: "invalid badgeId" }) };
      }
      ...
      if (existing.Item) {
        return { statusCode: 409, headers: getCorsHeaders(event), body: JSON.stringify({ message: "badge already awarded" }) };
      }
    ...
    // GET group LEADERBOARD - GET /communities/{name}/leaderboard
    if (method === "GET" && event.pathParameters && event.pathParameters.name && path.includes("/leaderboard")) {
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ leaderboard, timeframe, group })
      };
    }
    ...
    if (method === "GET" && path === "/leaderboard") {
      ...
      return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify({ leaderboard, metric })
      };
    }
